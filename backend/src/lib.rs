...
//! This crate is **fully out-of-process**: `apps/core` does not depend on it at all
//! (no path dep, no `recipes` cargo feature), it ships as the `ryu-recipes` sidecar
//! binary, and its `/api/recipes/*` surface reaches clients through the manifest's
//! `public_mount` over the generic ext-proxy. The workflow executor's
//! `Recipe`/`GhostAction` nodes no longer call [`run`]/[`extract_mcp_json`]; they
//! drive Core's own built-in `ghost__ghost_run` MCP tool through the shared
//! registry, so both lanes end at the same engine without a Rust edge between them.
//!
...
//!   through the [`RecipesHost`] trait. The implementor is **this crate's own
//!   `main.rs`** (`CoreCallback`), which POSTs each verb back to Core's generic
//!   `ghost.*` kernel capabilities (`/api/host/capability/ghost.{replay,recordStart,
//!   recordStatus,recordStop}`) and deserializes the JSON reply into
//!   [`RecorderStarted`]/[`RecorderStatus`]/[`RecorderStopped`]. Core installs
//!   nothing here — the boundary is plain JSON, pinned by the paired key-set tests
//!   in this crate and in `apps/core/src/recipes_host.rs`.
//!
//! ## App events (outbound)
...
/// no-op emitter when they are absent — the crate's own tests and any other host
/// that has not been handed a per-plugin `RYU_EXT_TOKEN`. So the events fire from
/// the `ryu-recipes` sidecar, the only deployment there is, and stay silent
/// elsewhere instead of failing. Note the corollary for the workflow executor's
/// `Recipe` node: it drives Core's built-in ghost MCP tool directly and never
/// enters this crate, so `replay.ended`/`replay.failed` do not fire on that lane.
/// They never did — Core has `RYU_CORE_PORT` but no `RYU_EXT_TOKEN` of its own, and
/// an event id namespaced to `@ryu/recipes` may only be raised by that plugin.
                ...
                .emit_with_notify(
                    ...
                    Some(
                        ryu_app_events::NotifyHint::info(
                            format!("Recipe “{name}” replay done"),
                            None,
                        )
                        .with_level("success"),
                    ),
        ...
        Err(e) => {
            emitter()
                .emit_with_notify(
                    ...
                    Some(
                        ryu_app_events::NotifyHint::info(
                            format!("Recipe “{name}” replay failed"),
                            Some(format!("{e:#}")),
                        )
                        .with_level("error"),
                    ),
...
mod tests {
    ...
    /// The other half of the Core↔sidecar wire pin. Core no longer links this
    /// crate, so it hands `/api/host/capability/ghost.record*` back as bare JSON
    /// built by `recipes_host.rs`'s `started_body`/`status_body`/`stopped_body`.
    /// These are those exact literals: if a key is renamed on either side, one of
    /// the two suites fails. The Core-side counterpart asserts the same key sets.
    #[test]
    fn recorder_shapes_deserialize_from_cores_json_bodies() {
        let started: RecorderStarted =
            serde_json::from_value(json!({ "started_at": "t0", "info": { "ok": true } })).unwrap();
        assert_eq!(started.started_at, "t0");
        assert_eq!(started.info, json!({ "ok": true }));

        let status: Option<RecorderStatus> = serde_json::from_value(
            json!({ "task": "t", "started_at": "t0", "status": Value::Null }),
        )
        .unwrap();
        let status = status.expect("active session");
        assert_eq!(status.task, "t");
        assert_eq!(status.status, Value::Null);

        // Core sends JSON `null` for the idle recorder.
        let idle: Option<RecorderStatus> = serde_json::from_value(Value::Null).unwrap();
        assert!(idle.is_none());

        let stopped: RecorderStopped = serde_json::from_value(
            json!({ "task": "t", "started_at": "t0", "payload": { "event_count": 0 } }),
        )
        .unwrap();
        assert_eq!(stopped.payload, json!({ "event_count": 0 }));
    }
